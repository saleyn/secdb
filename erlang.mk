...
ERL_APP_FILE := ebin/$(PROJECT).app
ERL_APP_SRC  := src/$(PROJECT).app.src
...
app:: erlc-include $(ERL_APP_FILE)
	...
	$(eval OLD_MODULES := $(shell [ ! -f $(ERL_APP_FILE) ] || sed -n \
		's/[[:space:]]*{modules,[[:space:]]*\[\(.*\)\]}.*$$/\1/p' $(ERL_APP_FILE)))
	@[ ! -f $(ERL_APP_FILE) -o $(ERL_APP_SRC) -nt $(ERL_APP_FILE) -o "$(APP_MODULES)" != "$(OLD_MODULES)" ] && \
...
erl_verbose_0 = @echo " ERL   " $(C_SRC_ENV);
erl_verbose = $(erl_verbose_$(V))

$(C_SRC_ENV):
	$(erl_verbose) erl -noshell -noinput -eval "file:write_file(\"$(C_SRC_ENV)\", \
		io_lib:format( \
