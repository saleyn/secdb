...
testdir() ->
  case application:get_env(secdb, testdir) of
    {ok, Value} ->
      Value;
    undefined ->
...
chunk_109_content() ->
  [
    ...
    #trade{timestamp=1343207273291, side=buy, price=12.33, volume=490, aggressor=undefined},
    #trade{timestamp=1343207273292, side=buy, price=12.23, volume=4600,aggressor=undefined},
    #trade{timestamp=1343207273293, side=buy, price=12.45, volume=400, aggressor=undefined},
    ...
    #trade{timestamp=1343207362719, side=buy, price=12.34, volume=200, aggressor=undefined},
    {md, 1343207382471, [{12.34, 300}, {12.195, 120}, {10.85, 1200}], [{12.435, 650}, {12.47,  950}, {12.67, 600}]}
  ].

chunk_110_content_1_trunc() ->
  [
    #trade{timestamp=1343207402486, side=buy, price=12.445, volume=300, aggressor=undefined},
...
chunk_110_content_2() ->
  [
    ...
    #trade{timestamp=1343207652486, side=buy, price=12.45, volume=200, aggressor=undefined}
  ].
...
append_events_to_file(File, Events) ->
  Opts = [{symbol, 'TEST'}, {date, {2012,7,25}}, {depth, 3}, {scale, 200}, {window_sec, 300}],
  {ok, S0} = secdb_appender:open(File, Opts),
  S1 = lists:foldl(fun(I, State) -> secdb_appender:append(I, State) end, S0, Events),
  ok = secdb:close(S1).
...
ensure_packets_equal(#trade{} = P1, #trade{} = P2) ->
  ?assertEqual(P1, P2);
...
packets_equal(#trade{timestamp = TS1, price = P1} = P1,
              #trade{timestamp = TS2, price = P2} = P2) ->
