...
file_info_test() ->
  ...
  secdb_appender:write_events(File, chunk_content('109') ++ chunk_content('110_1'), 
    [{symbol, 'TEST'}, {date, {2012,7,25}}, {depth, 3}, {scale, 200}, {chunk_size, 300}]),

  ?assertEqual([{date,{2012,7,25}},{scale,200},{depth,3}, {presence,{289,[109,110]}}],
      secdb_reader:file_info(File, [date, scale, depth, presence])),
  ?assertEqual([{candle,{12.33,12.45,12.23,12.445}}], secdb_reader:file_info(File, [candle])),
  file:delete(File).


candle_test() ->
  Root = secdb:get_value(root),
  ok = application:set_env(fix, root, tempdir()),
  file:delete(secdb_fs:path('TEST', "2012-07-25")),
  secdb:write_events('TEST', "2012-07-25", chunk_content('109') ++ chunk_content('110_1'), [{scale, 200}]),
