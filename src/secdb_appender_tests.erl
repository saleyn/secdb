...
fill_records(S0, Count, Step, TS) ->
  {ok, S1} = secdb:append({md, TS+0*Step, [{43.15, 50}], [{45.15, 50}]}, S0),
  {ok, S2} = secdb:append({md, TS+1*Step, [{43.09, 40}], [{45.05, 50}]}, S1),
  {ok, S3} = secdb:append({md, TS+2*Step, [{42.50, 20}], [{44.95, 20}]}, S2),
  {ok, S4} = secdb:append({md, TS+3*Step, [{42.05, 90}], [{43.15, 50}]}, S3),
  {ok, S5} = secdb:append({md, TS+4*Step, [{42.45, 50}], [{44.15, 40}]}, S4),
  {ok, S6} = secdb:append({md, TS+5*Step, [{41.55, 54}], [{44.15, 50}]}, S5),
  {ok, S7} = secdb:append({md, TS+6*Step, [{42.12, 10}], [{45.15, 30}]}, S6),
  {ok, S8} = secdb:append({md, TS+7*Step, [{42.80, 90}], [{43.15, 80}]}, S7),
  {ok, S9} = secdb:append({md, TS+8*Step, [{43.12, 20}], [{44.15, 50}]}, S8),
  {ok, S_} = secdb:append({md, TS+9*Step, [{43.45, 20}], [{45.15, 50}]}, S9),
