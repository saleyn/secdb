%%% @doc Binary row format for secdb
-module(secdb_format_v3).
...
get_first_packet(Buffer, #db{depth = Depth, last_bidask = LastBidAsk, last_timestamp = LastTimestamp, scale = Scale} = State) ->
  case packet_type(Buffer) of
    full_md ->
      ...
      {packet_from_mdentry(Timestamp, BidAsk, State), Tail,
        State#db{last_timestamp = Timestamp, last_bidask = BidAsk}};
    delta_md ->
...
format_header_value(symbol, Symbol) ->
...
read_all_packets(Buffer, #db{} = State0) when is_binary(Buffer) ->
...
packet_from_mdentry(Timestamp, BidAsk, #db{depth = Depth, scale = Scale}) ->
